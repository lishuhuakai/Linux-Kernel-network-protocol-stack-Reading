...
static int __init early_initrd(char *p)
{
	...
	if (*endp == ',') {
		...
		phys_initrd_start = start; /* 起始位置 */
		phys_initrd_size = size; /* 终止位置 */
	}
	return 0;
}
	...
	unsigned long *min, unsigned long *max_low, unsigned long *max_high)
{
	...
	*max_low = *max_high = 0;
    /* 这里其实就是要找到最大值与最小值而已
     * 最小值记录在min,max分为两个,max_low 低内存的最高值, max_high 包含高内存的最高值
     */
	for_each_nodebank(i, mi, node) { /* 遍历每一个membank */
		struct membank *bank = &mi->bank[i];
...
find_bootmap_pfn(int node, struct meminfo *mi, unsigned int bootmap_pages)
{
	...
	for_each_nodebank(i, mi, node) {
		...
		if (end - start >= bootmap_pages) {
			bootmap_pfn = start; /* 应该是将这块区域作为位图起始 */
			break;
		}
	}
 ...
 */
static void __init
bootmem_init_node(int node, struct meminfo *mi,
...
{
	struct meminfo *mi = &meminfo; /* meminfo记录了所有的内存块 */
	unsigned long min, max_low, max_high;
...
static void __init free_unused_memmap_node(int node, struct meminfo *mi)
{
	...
	for_each_nodebank(i, mi, node) {
		...
		bank_start = bank_pfn_start(bank); /* 起始页 */
 ...
 */
/* mem_init标记处mem_map中的空闲区域,并告诉我们还有多少内存可以使用
 *
 */
