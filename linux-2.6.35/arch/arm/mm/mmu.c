 ...
 */
static void __init
alloc_init_section(pgd_t *pgd, unsigned long addr,
...
static int __init early_vmalloc(char *arg)
{
	...
	if (vmalloc_reserve < SZ_16M) {
		vmalloc_reserve = SZ_16M; /* vmalloc最小为16M */
		printk(KERN_WARNING
...
sanity_check_meminfo(void)
{
	...
	for (i = 0, j = 0; i < meminfo.nr_banks; i++) {
		...
		*bank = meminfo.bank[i];
		 ...
		 * the vmalloc area.
		 * 检查memory bank和vmalloc area发生了部分重叠
		 */
