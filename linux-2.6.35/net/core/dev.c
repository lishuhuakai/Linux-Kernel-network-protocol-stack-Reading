...
static inline void __netif_reschedule(struct Qdisc *q)
{
	...
	*sd->output_queue_tailp = q;
	...
	raise_softirq_irqoff(NET_TX_SOFTIRQ); /* 这里激活了软中断 */
...
/* dev_hard_start_xmit将待输出的数据包提交给网络设备的输出接口，完成数据包的输出
 */
 ...
 *  将一个数据包传递至一个网络设备。调用者必须设定好要传递到的设备以及优先级
 *  并且构建好这个buff(即skb)
 *
...
int dev_queue_xmit(struct sk_buff *skb)
{
	...
	if (netif_needs_gso(dev, skb)) /* 如果是GSO数据包，而且设备支持GSO数据包处理 */
		goto gso;

	/* Convert a paged skb to linear, if required */
	/* 对于FRAGLIST类型的聚合分散IO数据包，如果输出网络设备不支持FRAGLIST类型的聚合分散IO
     * 则需要将其线性化，若线性化失败，丢弃数据包
	 */
...
gso:
	...
	q = rcu_dereference_bh(txq->qdisc); /* 获取出口流控的根流控对象 */

#ifdef CONFIG_NET_CLS_ACT
	skb->tc_verd = SET_TC_AT(skb->tc_verd, AT_EGRESS);
#endif
	if (q->enqueue) { /* 如果启用了Qos */
		rc = __dev_xmit_skb(skb, q, dev, txq); /* 使用出口流控发送数据包 */
		goto out;
	}
	...
	if (dev->flags & IFF_UP) {
		...
		if (txq->xmit_lock_owner != cpu) {
			...
			HARD_TX_LOCK(dev, txq, cpu); /* 执行加锁操作 */

			if (!netif_tx_queue_stopped(txq)) {
				/* 这个函数非常重要，调用网络设备来发送数据包*/
...
/* 数据包输出软中断例程，一旦被激活，便会遍历output_queue */
static void net_tx_action(struct softirq_action *h)
{
	...
	if (sd->output_queue) { /* 遍历output_queue */
		struct Qdisc *head;
...
{
	struct net_device *dev = skb->dev; /* 接收此数据包的网络设备 */
	...
	rxq = &dev->rx_queue; /* 接收队列 */

	q = rxq->qdisc;
	if (q != &noop_qdisc) {
		spin_lock(qdisc_lock(q));
		if (likely(!test_bit(__QDISC_STATE_DEACTIVATED, &q->state)))
			result = qdisc_enqueue_root(skb, q); /* 将包压入队列 */
 ...
 *  netif_receive_skb()是主要的接收数据的处理函数，它总是会成功的。
 *  传递的数据包可能会被协议丢掉。
 *
...
void napi_complete(struct napi_struct *n)
{
	...
	napi_gro_flush(n);
	local_irq_save(flags);
	__napi_complete(n); // 这里会将设备从cpu的poll_list上删除
	local_irq_restore(flags);
}
