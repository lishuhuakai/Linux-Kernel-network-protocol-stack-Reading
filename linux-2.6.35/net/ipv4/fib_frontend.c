 ...
 */
/* 通过netlink机制来操控路由
 * @param cmd 命令,如RTM_ADDROUTE等
 * @param type 路由类型, RTN_LOCAL -- 目的地址是到本机接口的路由
 * @param dst 目的地址
 * @param ifa ip地址块
 */
static void fib_magic(int cmd, int type, __be32 dst, int dst_len, struct in_ifaddr *ifa)
{
	...
	if (type == RTN_UNICAST)
		...
		tb = fib_new_table(net, RT_TABLE_LOCAL); /* 本地路由表 */
	...
	if (type != RTN_LOCAL)
		...
		cfg.fc_scope = RT_SCOPE_HOST; /* 到本机的路由 */
