			   ...
			   struct rtable **result)
{
	...
	out_dev = in_dev_get(FIB_RES_DEV(*res)); /* 下一跳对应的设备 */
	if (out_dev == NULL) {
		if (net_ratelimit())
			       ...
			       u8 tos, struct net_device *dev)
{
	...
	if (daddr == htonl(0xFFFFFFFF) || (saddr == 0 && daddr == 0)) /* 目的地址是广播地址 */
		goto brd_input;
	 ...
	 */
	if ((err = fib_lookup(net, &fl, &res)) != 0) { /* 查找对应的路由 */
...
local_input:
	rth = dst_alloc(&ipv4_dst_ops); /* 构建路由缓存块 */
	if (!rth)
		goto e_nobufs;
	...
	rth->rt_gateway	= daddr;
	rth->rt_spec_dst= spec_dst;
	rth->u.dst.input= ip_local_deliver; /* 将报文传递到本机 */
			    ...
			    unsigned flags)
{
	...
	rth->u.dst.output=ip_output; /* 对应的处理函数是ip_output */
	rth->u.dst.obsolete = -1;
