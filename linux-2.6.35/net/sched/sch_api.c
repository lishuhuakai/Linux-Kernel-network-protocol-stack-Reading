...
static struct Qdisc *qdisc_leaf(struct Qdisc *p, u32 classid)
{
	 ...
	 * prio_class_ops对应的是  如果是prio队列规则，则这个是获取父Qdisc中的22:4中的4
	 */
	cl = cops->get(p, classid);

	if (cl == 0)
		return NULL;
	/* 如果父为prio类型队列规程，获取父QDISC p中的第classid(低16位)的qdisc，
	 * prio_sched_data->queues[cl]
	 */
	leaf = cops->leaf(p, cl);
	cops->put(p, cl);
	return leaf;
...
/* 添加子队列规程的时候，通过classid来进行分类，从而决定把子Qdisc添加到父Qdisc的那个分类信息中，
 * 一般一个Qdisc带多个分类信息，可以参考prio_qdisc_ops中的get和graft接口，就好理解了
		       ...
		       struct Qdisc *new, struct Qdisc *old)
{
	...
	if (parent == NULL) { /* 如果是根队列规程，则把该新的队列规程和dev设备关联起来 */
		...
		for (i = 0; i < num_q; i++) {
			...
			/* 入口方向:ingress对应的是net_device dev中的rx_queue，
			 * 出口方向为:net_device dev中的_tx[i]
			 */
			if (!ingress)
	     ...
	     struct nlattr **tca, int *errp)
{
	...
	ops = qdisc_lookup_ops(kind); /* 根据种类来查找对应的处理函数 */
...
replay:
	...
	if ((dev = __dev_get_by_index(net, tcm->tcm_ifindex)) == NULL) /* 获取网络设备的抽象 */
		return -ENODEV;
	...
	if (clid) {
		if (clid != TC_H_ROOT) { /* 父节点不为根节点 */
			if (clid != TC_H_INGRESS) { /* 父节点不是入队列规则的根 */
				if ((p = qdisc_lookup(dev, TC_H_MAJ(clid))) == NULL)
					return -ENOENT;
 ...
 * 以使用句柄(handle)来命名；如果要建立一个类，可以使用类识别符(classid)来命名。
 *
 * remove，删除有某个句柄(handle)指定的QDisc，根QDisc(root)也可以删除。被删除QDisc
 * 上的所有子类以及附属于各个类的过滤器都会被自动删除。
 *
 * change，以替代的方式修改某些条目。除了句柄(handle)和祖先不能修改以外，change命令
 * 的语法和add命令相同。换句话说，change命令不能一定节点的位置。
 *
