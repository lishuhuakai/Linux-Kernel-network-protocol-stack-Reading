...
/* fib_nh存放着下一跳的路由地址 */
struct fib_nh {
	struct net_device	*nh_dev; /* 该路由表项输出的网络设备 */
	struct hlist_node	nh_hash; /* 用于将此entry插入到散列表中 */
	struct fib_info		*nh_parent; /* 指向所属的fib_info结构,fib_info也指向此结构 */
	unsigned		nh_flags;
...
/* 被许多路由所共享的信息 */
struct fib_info {
	struct hlist_node	fib_hash; /* 这个结构是一个嵌套结构，用于将这个entry插入到fib_info_hash表中 */
	struct hlist_node	fib_lhash; /* 同上，用于将这个entry插入到fib_info_laddrhash散列表中 */
	struct net		*fib_net;
	int			fib_treeref; /* 持有该entry的fib_node的数目 */
	atomic_t		fib_clntref; /* 由于路由查找成功而被持有的引用计数 */
	int			fib_dead; /* 标志路由项正在被删除 */
	unsigned		fib_flags; 
	int			fib_protocol;
	__be32			fib_prefsrc;
...
/* 路由表 */
struct fib_table {
	struct hlist_node tb_hlist; /* 将各个路由表，链接成一个双向链表 */
	u32		tb_id; /* 路由表标识 */
	int		tb_default;
