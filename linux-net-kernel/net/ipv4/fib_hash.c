...
struct fib_node {
	struct hlist_node	fn_hash; /* 用于指向下一条fib_node */
	struct list_head	fn_alias;
	__be32			fn_key;
	struct fib_alias        fn_embedded_alias;
};
/* fn_zone用于描述一组有着相同目的地址掩码长度的路由表项的集合 */
struct fn_zone {
	struct fn_zone		*fz_next;	/* Next not empty zone	*/
	struct hlist_head	*fz_hash;	/* Hash table pointer 指向存储该zone中路由项的散列表	*/
	int			fz_nent;	/* Number of entries 散列表中fib_node实例的数目	*/
...
int fib_table_insert(struct fib_table *tb, struct fib_config *cfg)
{
	...
	f = fib_find_node(fz, key); /* 根据key在zone中查找fib_node */
	...
	if (fa && fa->fa_tos == tos &&
		 ...
		 * duplicate routes
		 * 1. 找到type，scope,fib_info都匹配的表项，避免包含重复的路由信息
		 * 2. Find next 'fa' (or head), NLM_F_APPEND inserts before it
		 * 2. 找到下一个fa
		 */
		fa_match = NULL;
		fa_first = fa;
        /* 共享同一个fib_node实例的所有fib_alias实例都链接在一起 */
		...
		if (cfg->fc_nlflags & NLM_F_REPLACE) { /* 如果匹配了就要替换 */
	...
	new_fa = &f->fn_embedded_alias; /* 创建一个新的alias结构 */
