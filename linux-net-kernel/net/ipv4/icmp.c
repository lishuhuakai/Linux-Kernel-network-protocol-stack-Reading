...
struct icmp_control {
	void (*handler)(struct sk_buff *skb); /* 处理函数，用于处理icmp报文 */
	short   error;		/* This ICMP is classed as an error message */
};

static const struct icmp_control icmp_pointers[NR_ICMP_TYPES+1]; /* 这里就是icmp报文的handle */
...
/* 
 * ICMP差错报文的数据部分包括:原始数据报的IP首部再加上前8个字节的数据部分
 *  (2字节源端口+2字节目的端口+4字节序号)
 * 参考P229 樊东东
 */
...
int icmp_rcv(struct sk_buff *skb)
{
	...
	icmp_pointers[icmph->type].handler(skb); /* 这里直接根据icmp报文的类型来进行处理    */
...
/* 由于协议栈本身有发送ICMP数据报的需求，所以，需要在协议栈中创建内核态的原始套接字，
 * 用于发送ICMP数据报，这个事情在协议栈初始化时，
 * 由 icmp_init函数完成。它为每个CPU都创建一个icmp_socket，
 * 创建工作由sock_create_kern函数完成，
 * 创建流程跟应用层 创建socket完全一致。
 */
