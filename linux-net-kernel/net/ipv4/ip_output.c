...
int ip_fragment(struct sk_buff *skb, int (*output)(struct sk_buff *))
{
	...
	struct rtable *rt = skb_rtable(skb); /* 路由表项 */
	int err = 0;
	...
	hlen = iph->ihl * 4; /* ip头部的长度 */
	mtu = dst_mtu(&rt->u.dst) - hlen;	/* Size of data space */
#ifdef CONFIG_BRIDGE_NETFILTER
	if (skb->nf_bridge)
		mtu -= nf_bridge_mtu_reduction(skb);
#endif
	IPCB(skb)->flags |= IPSKB_FRAG_COMPLETE; /* 在分片之前先给IP数据报控制块设置此标志，表示完成分片 */

	/* When frag_list is given, use it. First, check its validity:
	 * some transformers could create wrong frag_list or break existing
	 * one, it is not prohibited. In this case fall back to copying.
	 * 
	 * 如果frag_list已经给定，使用它，首先要检验它的有效性，一些时候，传送者会构建出
	 * 错误的frag_list
	...
	if (skb_has_frags(skb)) {
		...
		int first_len = skb_pagelen(skb); /* 获取分片长度 */

		if (first_len - hlen > mtu ||
		    ((first_len - hlen) & 7) || /* 这里判断分片长度是否8字节对齐 */
		    (iph->frag_off & htons(IP_MF|IP_OFFSET)) ||
		    skb_cloned(skb))
			goto slow_path;

		skb_walk_frags(skb, frag) { /* 遍历所有分片 */
			/* Correct geometry. */
			if (frag->len > mtu ||
			    ((frag->len & 7) && frag->next) ||
			    skb_headroom(frag) < hlen) /* 这里判断skb预留的头部长度是否可以放下ip首部的数据 */
				goto slow_path_clean;

			/* Partially cloned skb? */
			if (skb_shared(frag)) /* 如果被克隆 */
				goto slow_path_clean;

			BUG_ON(frag->sk);
			if (skb->sk) { 
				frag->sk = skb->sk; /* 获取传输控制块 */
		...
		err = 0;
		offset = 0;
		frag = skb_shinfo(skb)->frag_list; /* 保存frag_list的指针 */
		skb_frag_list_init(skb);
		skb->data_len = first_len - skb_headlen(skb); 
		skb->len = first_len;
		iph->tot_len = htons(first_len);
		iph->frag_off = htons(IP_MF);
		ip_send_check(iph); /* 这里实际是计算ip首部校验和 */
		...
		for (;;) {
			...
			if (frag) {
				...
				__skb_push(frag, hlen); /* 预留ip首部的空间 */
				skb_reset_network_header(frag);
				memcpy(skb_network_header(frag), iph, hlen); /* 将skb的ip首部拷贝过来 */
				iph = ip_hdr(frag);
				iph->tot_len = htons(frag->len);
				ip_copy_metadata(frag, skb); /* 拷贝源数据 */
				if (offset == 0) /* 只有第一个分片需要调用ip_options_fragment */
					ip_options_fragment(frag);
				offset += skb->len - hlen;
				iph->frag_off = htons(offset>>3);
				if (frag->next != NULL)
					iph->frag_off |= htons(IP_MF); /* 分片还未完成 */
...
slow_path: /* 慢速分片 */
	...
	ptr = raw + hlen;		/* Where to start from 获取IP数据报中数据区指针 */

	/* for bridged IP traffic encapsulated inside f.e. a vlan header,
	 * we need to make room for the encapsulating header
	 */
	pad = nf_bridge_pad(skb);
	ll_rs = LL_RESERVED_SPACE_EXTRA(rt->u.dst.dev, pad); /* 二层首部长度 */
	mtu -= pad;
	...
	offset = (ntohs(iph->frag_off) & IP_OFFSET) << 3;  /* 获取IP首部中的片偏移值 */
	not_last_frag = iph->frag_off & htons(IP_MF); /* 是否为最后一个分片 */
	...
	while (left > 0) {
		...
		if (len > mtu)
			len = mtu; /* 每次发送mtu字节大小的数据 */
		/* IF: we are not sending upto and including the packet end
		   then align the next start on an eight byte boundary */
		if (len < left)	{ /* len >= left只有最后一个分片才会出现 */
			len &= ~7; /* 8字节对齐 */
		}
		/*
		 *	Allocate buffer.
		...
		ip_copy_metadata(skb2, skb); /* 填充类型 优先级 协议等 */
		...
		if (skb->sk)
			skb_set_owner_w(skb2, skb->sk); /* 设置新的skb的宿主 */
		...
		if (left > 0 || not_last_frag)
			iph->frag_off |= htons(IP_MF); /* 设置分片未完成标记 */
		ptr += len;
		offset += len;
