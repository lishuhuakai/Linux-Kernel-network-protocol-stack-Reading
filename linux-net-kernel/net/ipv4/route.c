	...
	for (rth = rcu_dereference(rt_hash_table[hash].chain); rth;
		...
		if ((((__force u32)rth->fl.fl4_dst ^ (__force u32)daddr) |
			...
			if (noref) {
				...
				skb_dst_set(skb, &rth->u.dst); /* 这里设置skb的路由表项 */
			}
				...
				const struct flowi *oldflp)
{
	...
	if (oldflp->fl4_src) { /* 源地址已知的情况下 */
		...
		if (oldflp->oif == 0 &&
			...
			fl.oif = dev_out->ifindex; /* 记录下输出设备的索引值 */
			goto make_route;
		}
	...
	if (!fl.fl4_dst) { /* 目的地址未知？ */
		fl.fl4_dst = fl.fl4_src;
		if (!fl.fl4_dst)
			fl.fl4_dst = fl.fl4_src = htonl(INADDR_LOOPBACK); /* 将目的地址设置为本机 */
		if (dev_out)
			dev_put(dev_out);
	...
	if (fib_lookup(net, &fl, &res)) { /* 查找路由表项 */
		res.fi = NULL;
		if (oldflp->oif) {
	...
	if (res.type == RTN_LOCAL) { /* 查找的数据报的目的地址为本机 */
		if (!fl.fl4_src)
			fl.fl4_src = fl.fl4_dst;
		if (dev_out)
			dev_put(dev_out);
		dev_out = net->loopback_dev; /* 将输出设备设置为本地回环设备 */
		dev_hold(dev_out);
