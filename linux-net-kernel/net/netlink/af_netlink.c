...
/* struct netlink_sock是struct sock的一个子类 */
			    ...
			    struct mutex *cb_mutex, int protocol)
{
	...
	sock->ops = &netlink_ops; /* 注意这里的操作集合 */
    /* 开辟struct netlink_sock空间大小，该结构中包括struct sock，是struct sock的扩展 */
 ...
 * 应用层创建netlink套接字的时候，会调用__sock_create，从而执行netlink_create函数
 * 执行该函数的用户程序，内核需要通过sock_register函数来注册后才会调用该函数  
 * PF_NETLINK协议族套接字会调用这个函数，因为sock_register(&netlink_family_ops);注册了netlink套接字

 * netlink_create建立对应客户端的套接口，这个是应用程序的套接字，应用程序创建socket套接字的时候，内核
 * 会有一个对应的内核套接字，就是客户端套接口
 * netlink_kernel_create为服务器端套接口，在各种协议类型模块初始化时调用的,, 而不是socket系统调用时调
 * 用的, 每个netlink协议初始化是只调用一次(MAX_LINKS里面的每一个值最多netlink_kernel_create一次)
 * 该函数在应用层创建套接字的时候，引起系统调用sys_socket，然后执行
 * int __sock_create(struct net *net, int family, int type, int protocol,函数的时候会调用该函数创建应用层对应的套接字
 */
			  ...
			  int kern)
{
    ...
    /* 应用程序创建socket套接字的时候，socket(PF_NETLINK, SOCK_RAW, NETLINK_FIREWALL)，
     * type字段只能用SOCK_RAW或者SOCK_DGRAM
     */
...
/*
 * 参考SYSCALL_DEFINE3(bind, int, fd, struct sockaddr __user *, umyaddr, int, addrlen)的
 ...
 */
/*
 * netlink_create建立对应客户端的套接口，这个是应用程序的套接字，应用程序创建socket套接字的时候，
 * 内核会有一个对应的内核套接字，就是客户端套接口
 * netlink_kernel_create为服务器端套接口，在各种协议类型模块初始化时调用的,
 * 而不是socket系统调用时调用的, 每个netlink协议初始化是只调用一次(MAX_LINKS里面的每一个值最多netlink_kernel_create一次)
		      ...
		      struct mutex *cb_mutex, struct module *module)
{
    ...
    /* 该函数的前三个参数和应用层的int socket(int domain, int type, int protocol);
     * 函数参数一致，并且domain和protocol值要一致
     */
	...
	netlink_table_grab();
	if (!nl_table[unit].registered) { /* 如果没有注册的话， */
 ...
 */
/* pf_inet的net_families[]为inet_family_ops，对应的套接口层ops参考inetsw_array中的inet_stream_ops 
 * inet_dgram_ops inet_sockraw_ops，传输层操作集分别为tcp_prot udp_prot raw_prot
    ...
    netlink_proto_init(void)
{
	...
	unsigned long limit;
	unsigned int order;
	int err = proto_register(&netlink_proto, 0); /* 注册netlink协议 */
	...
	nl_table = kcalloc(MAX_LINKS, sizeof(*nl_table), GFP_KERNEL);/* 开辟了MAX_LINKS个na_talbe指针空间 */
	if (!nl_table)
		goto panic;
	...
	sock_register(&netlink_family_ops); /* 注册socket */
    /* 文件系统增加netlink proc入口 */
