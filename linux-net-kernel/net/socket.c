...
/* 这里面会创建struct socket结构，然后会调用sock_register(&netlink_family_ops)注册的
 * netlink_create来完成struct netlink_sock的创建
 * 应用层创建套接字函数: int socket(int domain, int type, int protocol);
 * domain也就是family协议族  如果是netlink协议族，会执行netlink_create
 */
			 ...
			 struct socket **res, int kern)
{
	 ...
	 */
	if (net_families[family] == NULL) /* 好吧，这里直接找到注册的模块 */
